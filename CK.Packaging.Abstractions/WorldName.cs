...
public partial class WorldName : IEquatable<WorldName>
{
    ...
    {
        if( !IsValidRepositoryName( stackName ) ) throw new ArgumentException( "Invalid stackName." );
        _name = stackName;
        if( !String.IsNullOrWhiteSpace( ltsName ) )
        {
            if( !IsValidLTSName( ltsName ) ) throw new ArgumentException( "Invalid ltsName." );
            _ltsName = ltsName;
    ...
    {
        return TryParse( fullName ) ?? throw new ArgumentException( $"Invalid World full name '{fullName}'.", nameof( fullName ) );
    }
